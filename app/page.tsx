...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (lastMessage) {
      ...
      if (lastMessage.type === "odds_update") {
        ...
        toast.success(`Pipeline complete! ${lastMessage.data.verdicts} legs analyzed`);
        fetchDashboardData();
      }
  ...
  return (
    <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {data?.bankroll.drawdownPct > 0 && (
                <Badge variant={data.bankroll.drawdownPct > 0.15 ? "destructive" : "warning"}>
                  {data.bankroll.drawdownPct > 0.15 ? "⚠️" : "🔻"} {data.bankroll.drawdownPct}% drawdown
                </Badge>
              )}
            </div>
            <div className="mt-3">
              <div className="flex justify-between text-xs mb-1">
                ...
                <span>{(data?.bankroll.stakeMultiplier || 1) * 100}%</span>
              </div>
      ...
      {/* Recent Odds Moves */}
      {data?.recentOddsMoves && data.recentOddsMoves.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
          ...
          <CardContent>
            <div className="space-y-2">
              {data.recentOddsMoves.map((move, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                  <div>
                    <div className="font-medium">{move.match}</div>
                    <div className="text-sm text-muted-foreground">{move.selection}</div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className="text-sm text-muted-foreground line-through">{move.oldOdds.toFixed(2)}</div>
                      <div className="font-medium text-green-500">{move.newOdds.toFixed(2)}</div>
                    </div>
                    <Badge variant={move.movePct < 0 ? "default" : "destructive"}>
                      {move.movePct > 0 ? "+" : ""}{move.movePct.toFixed(1)}%
                    </Badge>
                  </div>
                </div>
              ))}
